...
CreateThread(function()
    while true do
        ...
        if not inJailzone then
            RSGCore.Functions.TriggerCallback('rsg-prison:server:prisonlocation', function(location)
                local RandomStartPosition = Config.Locations[location].spawns[math.random(1, #Config.Locations[location].spawns)]
                lib.notify(
                    { 
                ...
                Wait(1000)
                SetEntityCoords(cache.ped, vector3(RandomStartPosition.coords.x, RandomStartPosition.coords.y, RandomStartPosition.coords.z - 0.9))
                Wait(1000)
                DoScreenFadeIn(1000)
            end)
        end
...
RegisterNetEvent('RSGCore:Client:OnPlayerLoaded', function()
    RSGCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.metadata["injail"] > 0 then
            TriggerEvent("rsg-prison:client:Enter", PlayerData.metadata["injail"], 'Sisika')
        end
    end)
end)
...
-- send to jail
--------------------------
RegisterNetEvent('rsg-prison:client:Enter', function(time, location)
...
RegisterCommand('jailmeta', function()
    TriggerServerEvent('rsg-prison:server:jailloc')
end)
...
RegisterNetEvent('rsg-prison:client:freedom', function()
    ...
    DoScreenFadeOut(1000)
    Wait(3000)
    SetEntityCoords(cache.ped, Config.Locations[location]["outside"].coords.x, Config.Locations[location]["outside"].coords.y, Config.Locations[location]["outside"].coords.z, 0, 0, 0, false)
    SetEntityHeading(cache.ped, Config.Locations[location]["outside"].coords.w)
